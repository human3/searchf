...
class TextView:
    ...
    def _save(self) -> types.Status:
        '''Save the config'''
        if not self._config.has_filters():
            return 'No filters to save'
        if not self._config.dirty:
            return 'No change to save'
        status = self._store.save(self._config)
        self._config.dirty = False
        return f'Config saved in {status}'

    def _delete(self) -> types.Status:
        path = self._store.delete()
        if path:
            return f'{path} deleted'
        return 'Nothing to delete'

    def _load(self, goto_next: bool) -> types.Status:
        if not self._store.can_load():
            return 'Nothing to load'
        self._config, path = self._store.load(goto_next)
        self._sync(True)
        self._config.dirty = False
        return f'Config loaded from {path}'
...
layout of the view model.
    ...
    def execute(self, command: enums.TextViewCommand) -> types.Status:
        ...
        dispatch = {
            ...
            enums.TextViewCommand.SAVE:                  self._save,
            enums.TextViewCommand.DELETE:                self._delete,
            enums.TextViewCommand.LOAD_NEXT:             lambda: self._load(True),
            enums.TextViewCommand.LOAD_PREV:             lambda: self._load(False),
        }
...
class Views:
    ...
    def handle_key(self, key) -> Tuple[bool, types.Status]:
        ...
        keys_to_command = {
            ...
            ord('a'):              enums.TextViewCommand.SAVE,
            ord('A'):              enums.TextViewCommand.DELETE,
            ord('z'):              enums.TextViewCommand.LOAD_NEXT,
            ord('Z'):              enums.TextViewCommand.LOAD_PREV,
        }
